...
- **断网先落本地**：穿着打点、破损登记、修补登记、记录编辑在网络不可用时进入浏览器本地队列（localStorage），恢复网络后自动重放；
- **重放幂等**：创建类操作带 `clientOpId`（数据库唯一约束兜底），同一台设备重放几次都只建一条记录，并发重放也不会冒 500；
- **多端合并**：破损/修补记录带 `version` 乐观锁，任何修改（含状态流转）都会推进版本；编辑时带上 `baseVersion`，版本对不上返回 `409 VERSION_CONFLICT` 并附服务器当前记录；
- **冲突不静默**：合不上的修改进入「同步冲突」面板（顶栏红色角标），逐字段摆出「我的修改 vs 服务器当前值」，由用户选择放弃本地或以本地覆盖——绝不静默丢弃、也不偷偷覆盖。
...
- `tests/offline-sync.test.ts`：离线工作流——**clientOpId 重放幂等**（破损/修补，含并发重放不重复）、**按版本合并**（baseVersion 匹配才落库、过期版本 409 并带回服务器当前记录、并发编辑一胜一负、状态流转同样推进版本）
...
Playwright 端到端覆盖 8 条真实浏览器链路：注册建档、上传照片 + 画布标记、列表缩略图、打点穿着、**破损→修补→复检闭环**、全部页面冒烟（含图表与字典页）、布料建档 + 只读分享链接、断网打点/破损/修补离线入队与自动同步。
